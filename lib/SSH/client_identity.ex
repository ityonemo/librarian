defmodule SSH.ClientIdentity do
  ...
  def user_key(algorithm, connect_options) do
    ...
    with prv_opts when not is_nil(prv_opts) <- Keyword.get(connect_options, :key_cb_private),
         ...
         {:ok, pem_bin} <- expand_identity(identity),
  ...
  defp expand_identity(identity) do
    if File.regular?(identity |> Path.expand()) do
      File.read(identity |> Path.expand())
    else
      {:ok, identity}
    end
  end
  ...
  defp identity_pass_phrase(~c"ssh-dss"), do: :dsa_pass_phrase
  defp identity_pass_phrase(~c"ssh-rsa"), do: :rsa_pass_phrase
  defp identity_pass_phrase(~c"rsa-sha2-256"), do: :rsa_pass_phrase
  defp identity_pass_phrase(~c"rsa-sha2-384"), do: :rsa_pass_phrase
  defp identity_pass_phrase(~c"rsa-sha2-512"), do: :rsa_pass_phrase
  ...
  defp identity_pass_phrase(~c"ecdsa-sha2-" ++ _), do: :ecdsa_pass_phrase
